locals {
  cloudwatch_alarm_name_prefix = "${var.resource_group}/client-vpn-endpoint/${var.name}"
...
resource "aws_cloudwatch_metric_alarm" "client_vpn_cloudwatch_alarms" {
  ...
  dimensions = {
    Endpoint = module.client-vpn-endpoint.id
  }
