resource "aws_security_group" "alb_sg" {
  ...
  description = "Security group of ALB for customer ${var.customer} amd environment ${var.env}"
  vpc_id      = var.vpc_id
  ...
  egress {
    ...
    security_groups = [
      module.chart_museum_service.service_instance_sg_id
    ]
  }
...
resource "aws_lb_listener" "default_https" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = module.chart_museum_service.service_target_group_arn
  }
